...
* **Project Calico:** Core maintainer and contributor across Kubernetes networking, eBPF data planes, routing engines, egress control, and runtime observability pipelines.
* **Verdict:** Developed an open-source CLI network policy verifier and firewall rule engine in Go to simulate, validate, and debug complex multi-rule packet evaluations.
* **Fabricate:** Designed a programmable userspace TCP/SOCKS5 proxy featuring an embedded Lua scripting engine for dynamic traffic routing and header inspection.
...
* **Distributed Systems & Cloud:** Kubernetes, Docker, Linux (Ubuntu/Debian), AWS, Azure, GCP, Ansible, Git
